    ...
    const tableBody = (
      <tbody>
        {reports.map((report, index) => (
          <tr key={index}>
            <td className="px-4 py-2 border  text-stone-600">{report.location}</td>
            {report.hourlySale.map((sales, index) => (
              <td key={index} className="px-4 py-2 border  text-stone-600">
                {sales}
              </td>
            ))}
            <td className="px-4 py-2 border text-stone-600">
...
const tableFooter = (
  <tfoot className="bg-gray-200">
    <tr>
      ...
      <td className="px-4 py-2 border text-stone-600"></td>
    </tr>
  </tfoot>
);
